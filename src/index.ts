...
export function registerDesktop(pi: ExtensionAPI, controller: DesktopController, platform = process.platform) {
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        if (command === 'status' || command === '') {
          ctx.ui.notify(`${await versionStatus()}\ndesktop=${controller.isEnabled ? 'on' : 'off'}; macOS 14+ only. Diagnostics: npm run health (no capture/input).`, 'info');
          return;
        }
  ...
  pi.registerTool({
    ...
    description: 'Observe whole main display (PNG, longest edge <=1280). Returns timestamp, dimensions, one-use ref expiring after 120s. Requires user /desktop on. No AX/DOM/OCR.',
